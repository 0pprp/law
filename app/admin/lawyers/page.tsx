...
const ROLE_BADGE: Partial<Record<UserRole, 'navy' | 'info' | 'success' | 'orange' | 'purple'>> = {
  admin: 'purple',
  employee: 'info',
  accountant: 'success',
  lawyer: 'orange',
...
export default async function LawyersPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="المستخدمون"
        subtitle={`${profiles?.length ?? 0} مستخدم • ${activeCount} نشط`}
        actions={
      ...
      <div className="bg-white rounded-xl border border-[rgba(118,118,118,0.15)] shadow-sm overflow-hidden">
        {!profiles?.length ? (
          <EmptyState
            ...
            description="أضف أول مستخدم في النظام"
            action={<Link href="/admin/lawyers/new"><Button variant="primary" size="sm">+ إضافة مستخدم</Button></Link>}
          />
        ) : (
          <>
            {/* Desktop */}
                ...
                <TBody>
                  {profiles.map(user => (
                    <TR key={user.id}>
                      ...
                      <TD>
                        <LawyerActions userId={user.id} isActive={user.is_active} fullName={user.full_name} />
                      </TD>
                    </TR>
                  ))}
                </TBody>
