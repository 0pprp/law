...
interface TaskDefExpense {
  id?: string
  name: string
  max_amount: string
}

interface TaskDefExpenseRow {
  id: string
...
// ─── Edit Modal ───────────────────────────────────────────────────────────────
function EditModal({ def, reqFields, expenseRows, onClose, onSaved }: {
  def: TaskDef
  reqFields: ReqField[]
  expenseRows: TaskDefExpenseRow[]
  onClose: () => void
  onSaved: () => void
}) {
  const [label, setLabel] = useState(def.label)
  const [fee, setFee] = useState(String(def.fee_amount))
  const [expenseLines, setExpenseLines] = useState<TaskDefExpense[]>(() =>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={() => setExpenseLines(prev => [...prev, { name: '', max_amount: '' }])}
                className="text-[10px] font-bold text-sky-700 hover:underline">+ إضافة صرفية</button>
            </div>
            {expenseLines.length === 0 ? (
              <p className="text-xs text-[#767676] italic">لا صرفيات — لن تظهر نافذة الصرفيات للمحامي</p>
            ) : (
            ...
            <p className="text-[10px] text-[#767676] mt-1">تظهر للمحامي قبل الحقول الإلزامية — تُخصم من محفظة الصرفيات عند الاعتماد</p>
          </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#F3F1F2] border-b border-[rgba(118,118,118,0.1)]">
              <tr>
                ...
                <SortableTH variant="plain" sortKey="fields" activeKey={sortKey} direction={sortDirection} onCycle={cycleSort} className="text-right px-4 py-3 font-semibold text-[#767676] text-xs">الحقول الإلزامية</SortableTH>
