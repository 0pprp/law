...
import { isLegalManager } from '@/lib/permissions'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!profile || !isLegalManager(profile.role)) {
      return NextResponse.json({ error: 'صلاحيات غير كافية' }, { status: 403 })
    }

    const body = await request.json().catch(() => ({}))
    const { title, amount, notes } = body as {
      title?: string
    ...
    const result = await submitLawyerPayoutRequest(admin, {
      ...
      title: (title ?? 'طلب سحب').trim() || 'طلب سحب',
      amount: parsedAmount,
      notes: notes ?? null,
    ...
    console.error('[admin/legal-manager-payout-request]', e)
