...
/** روابط المهمة الهجينة لتعريف أساسي — قراءة للمدير أو المحامي/المندوب */
  ...
  try {
    ...
    const { data, error } = await admin
      ...
      .select(`
        ...
        is_optional,
        sort_order,
        linked:task_definitions!task_definition_links_linked_definition_id_fkey (
          label,
          fee_amount,
          task_type
        )
    ...
    const links: (LinkRow & { task_type?: string | null })[] = (data ?? []).map((row: any) => {
      ...
      return {
        ...
        task_type: linked?.task_type != null ? String(linked.task_type) : null,
      }
    })
    ...
    console.error('[task-definition-links GET]', e)
    return NextResponse.json({ error: 'حدث خطأ غير متوقع' }, { status: 500 })
  }
}

/**
 * استبدال روابط المهمة الهجينة:
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (hybridErr) {
      if (isMissingHybridSchema(hybridErr.message)) {
        return NextResponse.json({
          links: [],
          schemaReady: false,
          warning: 'أعمدة/جداول المهمة الهجينة غير مطبّقة بعد على قاعدة البيانات',
        })
      }
      return NextResponse.json({ error: hybridErr.message }, { status: 500 })
    }

    // إيقاف الهجين أو قائمة فارغة: امسح الروابط فقط
      ...
      if (delErr) {
        if (isMissingHybridSchema(delErr.message)) {
          return NextResponse.json({
            links: [],
            schemaReady: false,
            warning: 'جدول task_definition_links غير موجود بعد',
          })
        }
    ...
    const { error: insErr } = await admin.from('task_definition_links').insert(
    ...
    console.error('[task-definition-links POST]', e)
    return NextResponse.json({ error: 'حدث خطأ غير متوقع' }, { status: 500 })
  }
}
