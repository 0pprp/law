...
function StatCard({
  ...
  count: number
  label: string
  accent: 'blue' | 'teal' | 'emerald'
}) {
  const styles = {
    blue: {
      ring: 'ring-blue-100',
      num: 'text-blue-700',
      lbl: 'text-blue-600/90',
      bg: 'bg-blue-50/80',
    },
    teal: {
      ring: 'ring-[#2C8780]/15',
  ...
  return (
    <Link href={href} className="block group">
      ...
      >
        <p className={`text-3xl font-black tabular-nums leading-none ${styles.num}`}>{count}</p>
        <p className={`text-xs font-bold mt-2 ${styles.lbl}`}>{label}</p>
      </div>
    </Link>
  )
}
...
export default async function LawyerDashboardPage() {
  ...
  const [{ data: profile }, lawyerTasksRes, walletBalance] = await Promise.all([
  ...
  const counts = {
    ...
    completed: countLawyerAchievedTasks(allTasks),
  }

  const overdue = allTasks.filter(t => t.due_date && isTaskOverdue(t.due_date) && !['completed', 'closed', 'failed', 'approved'].includes(t.task_status))
    ...
    <div className="max-w-lg mx-auto px-4 pt-5 pb-24 space-y-5">
      ...
      {overdue.length > 0 && (
        <div className="bg-red-50 border border-red-100 rounded-3xl p-4 flex items-center gap-3 shadow-sm">
          ...
          <Link href="/lawyer/tasks?f=overdue" className="text-xs text-red-600 font-bold bg-white border border-red-200 px-3 py-1.5 rounded-xl whitespace-nowrap">عرض</Link>
        </div>
      )}
        ...
        <Link href="/lawyer/tasks?f=assignment_pending_acceptance" className="block">
          <div className="bg-amber-50 border border-amber-200 rounded-3xl p-4 flex items-center gap-3 shadow-sm active:scale-[0.99] transition-transform">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-amber-700 mt-0.5">{counts.pending_acceptance} مهمة — اقبل أو ارفض خلال 24 ساعة</p>
            </div>
            <span className="text-amber-900 font-black text-2xl tabular-nums">{counts.pending_acceptance}</span>
          </div>
        </Link>
      )}

      {/* Fee balance */}
      <div className="bg-white rounded-3xl border border-[#2C8780]/20 shadow-sm p-5 flex items-center justify-between gap-4">
        <div>
          <p className="text-[11px] font-bold text-[#767676] mb-1">رصيد الأتعاب</p>
          <p className="text-2xl font-black text-[#2C8780] leading-tight tabular-nums" dir="ltr">{fmtMoney(feeBalance)}</p>
        </div>
        <div className="w-12 h-12 rounded-2xl bg-[#2C8780]/10 flex items-center justify-center shrink-0">
          <svg className="w-6 h-6 text-[#2C8780]" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatCard href="/lawyer/tasks?f=new" count={counts.new} label="جديدة" accent="blue" />
        ...
        {latestTasks.length === 0 ? (
          ...
          <div className="px-3 pb-3 space-y-2">
            {latestTasks.map((task: any) => {
              ...
              const achieved = isLawyerAchievedTask(task.task_status)
              return (
                <Link key={task.id} href={`/lawyer/tasks/${task.id}`} className="block">
                  <div className={`rounded-2xl border p-4 transition-colors active:bg-[#2C8780]/[0.03] ${isOverdue ? 'border-red-200 bg-red-50/40' : isLastDay ? 'border-amber-200 bg-amber-50/40' : 'border-[rgba(118,118,118,0.1)] bg-[#F3F1F2]/50'}`}>
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <Badge variant={achieved ? 'success' : (STATUS_BADGE[task.task_status as TaskStatus] ?? 'default')}>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-[11px] text-[#767676]">
                      ...
                      {task.due_date && (
                        <span className={isOverdue ? 'text-red-500 font-semibold' : isLastDay ? 'text-amber-700 font-semibold' : ''} dir="ltr">
                          📅 {fmtDate(task.due_date)}{isLastDay ? ' · آخر يوم' : ''}
                        </span>
                      )}
                    </div>
                  </div>
