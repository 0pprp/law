...
const sections = [
  {
    items: [
      ...
      { label: 'جاري التسديد', href: '/admin/payment-follow-up', exact: true, icon: (
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'ترشيح اسم', href: '/admin/nominations/new', icon: (
        <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/></svg>
      )},
      { label: 'الأسماء التي تحتاج مراقبة', href: '/admin/special-statuses', icon: (
        <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z"/></svg>
      )},
      ...
      { label: 'المحامين', href: '/admin/lawyer-stats', icon: (
        <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
      )},
        ...
        <>
          ...
          <div className="absolute left-0 top-full mt-2 w-72 bg-white rounded-xl border border-[rgba(118,118,118,0.15)] shadow-lg z-50 overflow-hidden">
            ...
            <div className="py-1">
              ...
              {financeVisible && financePending > 0 && (
                <Link
                  href="/admin/finance"
                  onClick={onClose}
                  className="flex items-center gap-3 px-4 py-3 hover:bg-[#F8F7F8] transition-colors"
              ...
              {financeVisible && counts.pendingExpenses > 0 && (
                <Link
              ...
              {financeVisible && counts.pendingExpensesByType.map(item => (
                <Link
                  key={item.type}
...
function useAdminNotifications(branchId: string | null) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { void load(false) }, 120_000)
    return () => {
...
export default function AdminShell(props: AdminShellProps) {
  const role = (props.userRole as UserRole) || 'employee'
  return (
    <BranchProvider
...
function AdminShellInner({
  ...
  const counts = useAdminNotifications(branchId)
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 scrollbar-none">
        {sections.map((section, si) => {
          const items = section.items.filter(item => {
            if (item.href === '/admin/nominations/new' || item.href.startsWith('/admin/nominations')) {
              return canNominateInstantCase(userRole, accountantType)
            }
            return isNavVisibleForRole(item.href, userRole)
          })
  ...
  return (
    <div className="h-screen overflow-hidden" dir="rtl">
        ...
        <aside className="hidden lg:flex fixed top-0 right-0 bottom-0 w-64 z-30 bg-[#231F20] flex-col">
          {SidebarContent}
        </aside>
        ...
        <div className="h-screen overflow-y-auto overflow-x-hidden flex flex-col lg:mr-64 min-w-0">
          <header className="sticky top-0 z-20 bg-white border-b border-[rgba(118,118,118,0.1)] min-h-[3.75rem] sm:min-h-16 flex items-center px-4 sm:px-5 lg:px-8 gap-3 shadow-[0_1px_3px_rgba(0,0,0,0.04)] shrink-0">
            ...
            <div className="hidden lg:flex items-center gap-2 text-sm text-[#454042] font-medium">
          ...
          <main className="flex-1 p-4 sm:p-5 lg:p-7 xl:p-8 min-w-0 bg-[#F8F7F8]">
