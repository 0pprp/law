...
export default function LawyerStationerySummary({ filesBalance, stampsBalance, compact }: Props) {
  if (compact) {
    return (
      <div className="bg-white border border-emerald-200 rounded-xl px-4 py-2.5 shadow-sm">
        <p className="text-[10px] text-slate-400 font-medium mb-1">{STATIONERY_WALLET_LABEL}</p>
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[10px] text-slate-400">الفايلات</p>
            <p className="font-black text-sm text-emerald-700 tabular-nums">{filesBalance}</p>
          </div>
          <div>
            <p className="text-[10px] text-slate-400">الطوابع</p>
    ...
    <div className="bg-white rounded-3xl border border-emerald-200 shadow-sm p-5 flex items-center justify-between gap-3">
      <div className="min-w-0 flex-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-[10px] text-slate-400 mb-0.5">رقم الفايلات</p>
            <p className="text-xl font-black text-emerald-700 leading-tight tabular-nums">{filesBalance}</p>
          </div>
          <div>
