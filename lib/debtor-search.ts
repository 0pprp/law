...
export const DEBTOR_SEARCH_PLACEHOLDER = 'بحث بالاسم أو الهاتف أو رقم الوصل أو رقم المعاملة...'
...
export const DEBTOR_DEFAULT_SELECT =
  'id, full_name, phone, receipt_number, transaction_number, governorate'

export const DEBTOR_TASK_SELECT =
  'id, full_name, phone, governorate, receipt_type, receipt_number, transaction_number, remaining_amount, required_amount, has_contract, case_status, case_type, current_task_id'
...
export function debtorMatchesSearch(debtor: DebtorSearchFields, query: string): boolean {
  ...
  const txn = (debtor.transaction_number ?? '').toLowerCase()
  ...
  if (txn.includes(q)) return true
  if (qDigits && digitsOnly(txn).includes(qDigits)) return true
...
export function debtorSearchOrFilter(term: string): string {
  const s = term.trim()
  return `full_name.ilike.%${s}%,phone.ilike.%${s}%,receipt_number.ilike.%${s}%,transaction_number.ilike.%${s}%`
}

export function debtorSelectHint(
  debtor: DebtorSearchFields & { governorate?: string | null },
): string | undefined {
  const parts = [debtor.phone, debtor.transaction_number, debtor.receipt_number, debtor.governorate].filter(Boolean)
...
export async function fetchDebtorsBySearch(
  ...
  if (error) {
    const select = options?.select ?? DEBTOR_DEFAULT_SELECT
    if (select.includes('transaction_number')) {
      let retry = supabase
        .from('debtors')
        .select(select.replace(', transaction_number', '').replace('transaction_number, ', ''))
      if (options?.branchId) retry = (retry as any).eq('branch_id', options.branchId)
      if (options?.branchListId) retry = (retry as any).eq('branch_list_id', options.branchListId)
      if (options?.caseType === 'civil' || options?.caseType === 'criminal') {
        retry = (retry as any).eq('case_type', options.caseType)
      }
      const fb = await retry
        .or(`full_name.ilike.%${trimmed}%,phone.ilike.%${trimmed}%,receipt_number.ilike.%${trimmed}%`)
        .order('full_name')
        .limit(options?.limit ?? DEBTOR_SEARCH_RESULT_LIMIT)
      if (!fb.error) return (fb.data ?? []) as unknown as DebtorSearchRow[]
    }
    console.error('[fetchDebtorsBySearch]', error.message ?? error)
    return []
  }
  return (data ?? []) as unknown as DebtorSearchRow[]
}
