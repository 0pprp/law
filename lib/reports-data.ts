...
async function sumRequiredAmount(
  ...
  while (true) {
    ...
    if (debtorId) q = q.eq('id', debtorId)
...
async function fetchAchievementTasks(
  ...
  while (true) {
    ...
    const { debtorId, lawyerId, dateFrom, dateTo } = filters
    if (debtorId) q = q.eq('debtor_id', debtorId)
...
async function countOpenTasks(
  ...
  while (true) {
    ...
    if (filters.debtorId) q = q.eq('debtor_id', filters.debtorId)
...
export async function fetchReportSnapshot(
  ...
  const [lawyersRes, taskDefsRes, closedRes] = await Promise.all([
    supabase
      .from('profiles')
  ...
  const [
    ...
    sumColumnChunked(supabase, 'debtor_payments', branchId, 'amount', 'payment_date', filters, {
      lawyerCol: 'lawyer_id',
    }),
    sumColumnChunked(supabase, 'expenses', branchId, 'amount', 'expense_date', filters),
    sumRequiredAmount(supabase, branchId, filters.debtorId || undefined),
    fetchAchievementTasks(supabase, branchId, filters),
    countOpenTasks(supabase, branchId, filters),
