...
export function canStaffReadBranch(
  ...
  if (isAdmin(profile.role) || profile.role === 'employee' || isAnyLegalManager(profile.role)) return true
  ...
  if (isBranchManager(profile.role)) return profile.branch_id === targetBranchId
  return false
}

/** كتابة بيانات فرع (إضافة مدين / إعدادات) — مسؤولو الأقسام يكتبون ضمن نطاق قسمهم فقط */
