-- Credit lawyer fees at task completion approval (not on next-task transition).
-- Idempotent: one positive fee credit per task reference_id.

ALTER TABLE lawyer_wallet_transactions
  ADD COLUMN IF NOT EXISTS debtor_id uuid REFERENCES debtors(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS task_definition_id uuid REFERENCES task_definitions(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS source text;

CREATE UNIQUE INDEX IF NOT EXISTS idx_lawyer_wallet_task_fee_once
  ON lawyer_wallet_transactions (reference_id)
  WHERE wallet = 'fees' AND amount > 0 AND reference_id IS NOT NULL;
...
GRANT EXECUTE ON FUNCTION credit_task_completion_fee(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION credit_task_completion_fee(uuid, uuid) TO service_role;
