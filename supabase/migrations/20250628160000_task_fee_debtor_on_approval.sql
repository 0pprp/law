-- Credit lawyer fees + debtor lawyer_fees/required_amount atomically on task approval.
...
BEGIN
  ...
  IF v_task.debtor_id IS NOT NULL THEN
    UPDATE debtors
    SET lawyer_fees = COALESCE(lawyer_fees, 0) + v_fee
    WHERE id = v_task.debtor_id;
  END IF;
