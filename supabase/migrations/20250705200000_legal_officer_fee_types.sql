-- أنواع حركات جديدة: نسبة مسؤول القانونية + خصم صرفيات المحامي
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'wallet_transaction_type')
     AND NOT EXISTS (
       ...
       WHERE t.typname = 'wallet_transaction_type' AND e.enumlabel = 'legal_manager_percentage_fee'
     ) THEN
    EXECUTE 'ALTER TYPE wallet_transaction_type ADD VALUE ''legal_manager_percentage_fee''';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'wallet_transaction_type')
     AND NOT EXISTS (
       SELECT 1 FROM pg_type t
       JOIN pg_enum e ON t.oid = e.enumtypid
       WHERE t.typname = 'wallet_transaction_type' AND e.enumlabel = 'lawyer_expense_wallet_deduction'
     ) THEN
    EXECUTE 'ALTER TYPE wallet_transaction_type ADD VALUE ''lawyer_expense_wallet_deduction''';
  END IF;
END $$;
