-- RC security + money integrity hotfix
-- 1) Revoke dangerous SECURITY DEFINER RPCs from authenticated
-- 2) Prevent lawyers from self-approving / minting fees via RLS UPDATE
-- 3) Fix required_amount sync to use stable base (not live remaining after payments)
...
-- ═══════════════════════════════════════════════════════════
-- 2) Lawyer task UPDATE — block financial/status escalation
-- ═══════════════════════════════════════════════════════════
...
BEGIN
  -- Only constrain lawyer/delegate self-updates (staff/service bypass)
  SELECT role INTO v_role FROM public.profiles WHERE id = auth.uid();
  IF v_role IS NULL OR v_role NOT IN ('lawyer', 'delegate', 'محامي', 'attorney') THEN
    RETURN NEW;
  END IF;

  -- Must remain assigned to self (or clear assignment for rejection flow only)
  ...
  -- Never allow self-service fee / reward tampering
  ...
  -- Allowed status transitions for field workers only
...
-- ═══════════════════════════════════════════════════════════
-- 3) Required amount — stable base + resync remaining
-- ═══════════════════════════════════════════════════════════
-- Base = required − expenses − penalty (recovers receipt remaining even after payments).
-- Never use live remaining_amount (which already subtracts payments).
...
-- One-time repair for debtors whose required was corrupted by using live remaining
UPDATE public.debtors d
SET
...
-- ═══════════════════════════════════════════════════════════
-- 4) payment_follow_up — block balance/status tampering via RLS UPDATE
-- ═══════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- PFU may only touch payment metadata while staying in payment_in_progress
