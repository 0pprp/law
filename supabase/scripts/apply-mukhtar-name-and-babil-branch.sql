-- تطبيق يدوي آمن: «اسم المختار» الاختياري + فرع «بابل».
-- بيانات اسم المختار تُحفظ في tasks.completion_data بالمفتاح mukhtar_name.

UPDATE public.task_required_fields rf
SET field_type = 'text', field_label = 'اسم المختار', is_required = false
...
SELECT
  d.id, 'mukhtar_name', 'text', 'اسم المختار', false,
  COALESCE((
...
WHERE d.is_active = true
  ...
  AND NOT EXISTS (
    ...
    WHERE rf.task_definition_id = d.id AND rf.field_key = 'mukhtar_name'
  );

UPDATE public.branches SET is_active = true WHERE trim(name) = 'بابل';

INSERT INTO public.branches (name, city, is_active)
SELECT 'بابل', 'بابل', true
WHERE NOT EXISTS (SELECT 1 FROM public.branches WHERE trim(name) = 'بابل');
...
BEGIN
  SELECT id INTO v_source_branch_id FROM public.branches WHERE name = 'بغداد الكرخ' LIMIT 1;
  SELECT id INTO v_target_branch_id FROM public.branches WHERE trim(name) = 'بابل' LIMIT 1;
  IF v_source_branch_id IS NULL OR v_target_branch_id IS NULL THEN RETURN; END IF;

  FOR v_source_def IN
    SELECT task_type, label, fee_amount, sort_order, is_active, case_type
    FROM public.task_definitions WHERE branch_id = v_source_branch_id
  LOOP
    ...
    SELECT v_target_def_id, rf.field_key, rf.field_type, rf.field_label, rf.is_required, rf.sort_order
    FROM public.task_required_fields rf
    JOIN public.task_definitions source_definition ON source_definition.id = rf.task_definition_id
    ...
    INSERT INTO public.task_definition_expenses (task_definition_id, name, max_amount, sort_order)
    SELECT v_target_def_id, expense.name, expense.max_amount, expense.sort_order
    FROM public.task_definition_expenses expense
    JOIN public.task_definitions source_definition ON source_definition.id = expense.task_definition_id
    WHERE source_definition.branch_id = v_source_branch_id
      AND source_definition.task_type::text = v_source_def.task_type::text
      AND NOT EXISTS (
